...
impl MakepadSettings {
    pub fn initial() -> Self {
        Self {
                ...
                cfg.insert(
                    ...
                    HubBuilderConfig {
                        ...
                        workspaces: {
                            ...
                            workspace.insert("zaplib".to_string(), ".".to_string());
                            workspace
                        },
                    },
                );
            ...
            sync: {
                //sync.insert("main/zaplib".to_string(), vec!["windows/zaplib".to_string()]);
                HashMap::new()
            },
            builds: vec![BuildTarget {
                builder: "main".to_string(),
                workspace: "zaplib".to_string(),
...
impl MakepadStorage {
    ...
    fn file_path_to_live_path(fp: &str) -> String {
        if fp.starts_with("main/zaplib/") {
            fp["main/zaplib/".len()..].to_string()
        } else {
            fp.to_string()
        }
    }
    ...
    pub fn text_buffer_from_path(&mut self, cx: &mut Cx, path: &str) -> &mut MakepadTextBuffer {
        if let Some(tb_id) = self.text_buffer_path_to_id.get(path) {
            ...
            if let Ok(utf8_data) = zaplib::universal_file::read_to_string(&live_path) {
    ...
    pub fn handle_hub_msg(&mut self, cx: &mut Cx, htc: &FromHubMsg, makepad_windows: &mut Vec<ZapWindow>) {
        let hub_ui = self.hub_ui.as_mut().unwrap();
