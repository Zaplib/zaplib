...
Of course, WebAssembly is a much younger technology than Javascript. Both the implementation and the APIs of WebAssembly have a lot of room to grow, so if we're comparing both language's _potential_ we can't just look at current state of things. But it's hard to make predictions about the future, so let's see where we stand today.
